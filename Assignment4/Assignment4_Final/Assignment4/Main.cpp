...
int main()
{
	...
	//tested graveyard, fully functional
	//master->getTim()->setHealth(1);
	//master->getHar()->setHealth(1);
	while(master->alive()&&master->getCaptured()==false)
	{
		...
		//Caught all special cases where the heros would get stuck 
		//and cause seg faults <-not included in assignment
		//description.	
		...
		if((master->getHar()->getX()==24&&master->getHar()->getY()==5)&&master->getTim()->getHealth()==0) break;
	...
	//cout<<master->getTim()->getHealth()<<endl;
	//cout<<master->getHar()->getHealth()<<endl;
	return 0;
