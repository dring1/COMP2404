...
void Controller::update()
{
	...
	while(temp != 0)
	{
		if(temp->getData()->getX()<0||temp->getData()->getX()>24)
			WarriorList->pop(temp->getData());
	...
	//if timmy or harold are dead mark the spot
	if(timmy->getHealth()>30||timmy->getHealth()==0){
		timmy->setHealth(0);
		tDeath_x=timmy->getX();
		tDeath_y=timmy->getY();
	}
	if(harold->getHealth()>30||harold->getHealth()==0){
		harold->setHealth(0);
		hDeath_x=harold->getX();
		hDeath_y=harold->getY();
	}
}
