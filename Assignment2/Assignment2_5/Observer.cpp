...
Observer::Observer()
{
	plays = 100;//adjust this value to change the number of runs!
...
void Observer::battle2(PirateLine battle)//tortoise yields swords
{
	//cout<<"  " <<battle.empty()<<endl;
	//cout<<"  " <<tort1.getHealth()<<endl;
	//cout<<"  " <<hare1.getHealth()<<endl;
	hare2.swordPlay(true);//tort starts at head and has sword
	while((tort2.getHealth()>0||hare2.getHealth()>0)&&battle.empty()==false)//continue fighitng so long as both heroes arend dead
	{
		if(hare2.getHealth()>0)//check if tort is still alive
		{
			...
			if(battle.front()->getHealth()>0)//if pirate isnt dead strike back!
			{
				...
				hare2.damage(pDamage);//damage the tort
			} 
			else
			{
				battle.popFront();//if the pirate died pop it off the front
	...
	if(tort2.getHealth()==0&&hare2.getHealth()==0)//increment stat accordingly
	{
		hFail++;
	}
